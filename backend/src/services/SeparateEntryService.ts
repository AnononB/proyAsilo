import { v4 as uuidv4 } from 'uuid';
import { EntryRequest, ID } from '../types';
import { query, queryOne, execute, getDatabase } from '../database/database';
import { patientMedicationService } from './PatientMedicationService';
import { medicationService } from './MedicationService';

interface EntryFilters {
  type?: "entrada" | "salida" | "caducidad";
  patientId?: ID;
}

export class SeparateEntryService {
  // Generar folio según el tipo
  private async getNextFolio(type: "entrada" | "salida" | "caducidad"): Promise<string> {
    const year = new Date().getFullYear();
    const prefix = type === "entrada" ? "E" : type === "salida" ? "S" : "C";
    
    let tableName = "";
    let dateColumn = "";
    if (type === "entrada") {
      tableName = "entradas";
      dateColumn = "fecha_creacion";
    } else if (type === "salida") {
      tableName = "salidas";
      dateColumn = "fecha_creacion";
    } else {
      tableName = "caducidades";
      dateColumn = "fecha_registro";
    }
    
    const result = await queryOne<{ total?: number }>(`
      SELECT COUNT(*) AS total
      FROM ${tableName}
      WHERE YEAR(${dateColumn}) = @year
    `, { year });
    const next = (result?.total || 0) + 1;
    return `${prefix}-${year}-${String(next).padStart(4, '0')}`;
  }

  // Obtener fecha de entrada original para un medicamento
  private async getOriginalEntryDate(medicationId: ID): Promise<string | null> {
    try {
      const result = await queryOne<{ fecha_creacion?: string }>(`
        SELECT TOP 1 e.fecha_creacion
        FROM entradas e
        INNER JOIN entrada_items ei ON e.id = ei.entrada_id
        WHERE ei.medicamento_id = @medicationId
        ORDER BY e.fecha_creacion ASC
      `, { medicationId });
      return result?.fecha_creacion || null;
    } catch (error) {
      console.error('Error obteniendo fecha de entrada original:', error);
      return null;
    }
  }

  // Listar según el tipo
  async listEntryRequests(filters?: EntryFilters): Promise<EntryRequest[]> {
    const { type } = filters || {};
    
    if (!type) {
      // Combinar todas las tablas cuando no hay filtro
      const [entradas, salidas, caducidades] = await Promise.all([
        this.listEntradas(),
        this.listSalidas(),
        this.listCaducidades()
      ]);
      return [...entradas, ...salidas, ...caducidades].sort((a, b) => 
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
    }
    
    if (type === "entrada") return this.listEntradas();
    if (type === "salida") return this.listSalidas(filters?.patientId);
    if (type === "caducidad") return this.listCaducidades();
    
    return [];
  }

  private async listEntradas(): Promise<EntryRequest[]> {
    const entries = await query<any>(`
      SELECT 
        id,
        folio,
        fecha_creacion as createdAt,
        estado as status,
        comentario as comment
      FROM entradas
      ORDER BY fecha_creacion DESC
    `);
    
    const result: EntryRequest[] = [];
    for (const entry of entries) {
      const items = await this.getEntradaItems(entry.id);
      result.push({
        id: entry.id,
        folio: entry.folio,
        type: "entrada",
        patientId: "", // Entradas no tienen paciente
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        comment: entry.comment
      });
    }
    return result;
  }

  private async listSalidas(patientId?: ID): Promise<EntryRequest[]> {
    let sql = `
      SELECT 
        s.id,
        s.folio,
        s.paciente_id as patientId,
        s.fecha_creacion as createdAt,
        s.estado as status,
        s.fecha_vencimiento as dueDate,
        s.comentario as comment
      FROM salidas s
    `;
    const params: Record<string, any> = {};
    
    if (patientId) {
      sql += ` WHERE s.paciente_id = @patientId`;
      params.patientId = patientId;
    }
    
    sql += ` ORDER BY s.fecha_creacion DESC`;
    
    const entries = await query<any>(sql, params);
    
    const result: EntryRequest[] = [];
    for (const entry of entries) {
      const items = await this.getSalidaItems(entry.id);
      result.push({
        id: entry.id,
        folio: entry.folio,
        type: "salida",
        patientId: entry.patientId,
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        dueDate: entry.dueDate,
        comment: entry.comment
      });
    }
    return result;
  }

  private async listCaducidades(): Promise<EntryRequest[]> {
    const entries = await query<any>(`
      SELECT 
        id,
        folio,
        fecha_registro as createdAt,
        estado as status,
        comentario as comment
      FROM caducidades
      ORDER BY fecha_registro DESC
    `);
    
    const result: EntryRequest[] = [];
    for (const entry of entries) {
      const items = await this.getCaducidadItems(entry.id);
      result.push({
        id: entry.id,
        folio: entry.folio,
        type: "caducidad",
        patientId: "", // Caducidades no tienen paciente
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        comment: entry.comment
      });
    }
    return result;
  }

  private async getEntradaItems(entradaId: ID) {
    try {
      // Intentar seleccionar con nombre_medicamento, unidad, fecha_caducidad y dosis_sugerida (si existen las columnas)
      return await query<{ medicationId: ID; qty: number; medicationName?: string; unit?: string; fechaCaducidad?: string; dosisRecomendada?: string }>(`
        SELECT 
          ei.medicamento_id as medicationId,
          ei.cantidad as qty,
          COALESCE(ei.nombre_medicamento, m.nombre) as medicationName,
          COALESCE(ei.unidad, m.unidad) as unit,
          ei.fecha_caducidad as fechaCaducidad,
          ei.dosis_sugerida as dosisRecomendada
        FROM entrada_items ei
        LEFT JOIN medications m ON ei.medicamento_id = m.id
        WHERE ei.entrada_id = @entradaId
      `, { entradaId });
    } catch (error: any) {
      // Si falla por columnas faltantes, intentar sin dosis_sugerida
      if (error.message && (error.message.includes('dosis_sugerida') || error.message.includes('Invalid column name'))) {
        try {
          return await query<{ medicationId: ID; qty: number; medicationName?: string; unit?: string; fechaCaducidad?: string; dosisRecomendada?: string }>(`
            SELECT 
              ei.medicamento_id as medicationId,
              ei.cantidad as qty,
              COALESCE(ei.nombre_medicamento, m.nombre) as medicationName,
              COALESCE(ei.unidad, m.unidad) as unit,
              ei.fecha_caducidad as fechaCaducidad,
              NULL as dosisRecomendada
            FROM entrada_items ei
            LEFT JOIN medications m ON ei.medicamento_id = m.id
            WHERE ei.entrada_id = @entradaId
          `, { entradaId });
        } catch (error2: any) {
          // Si también falla por otras columnas, usar JOIN con medications
          if (error2.message && (error2.message.includes('nombre_medicamento') || error2.message.includes('unidad') || error2.message.includes('fecha_caducidad') || error2.message.includes('Invalid column name'))) {
            return await query<{ medicationId: ID; qty: number; medicationName?: string; unit?: string; fechaCaducidad?: string; dosisRecomendada?: string }>(`
              SELECT 
                ei.medicamento_id as medicationId,
                ei.cantidad as qty,
                m.nombre as medicationName,
                m.unidad as unit,
                m.fecha_vencimiento as fechaCaducidad,
                NULL as dosisRecomendada
              FROM entrada_items ei
              LEFT JOIN medications m ON ei.medicamento_id = m.id
              WHERE ei.entrada_id = @entradaId
            `, { entradaId });
          }
          throw error2;
        }
      }
      throw error;
    }
  }

  private async getSalidaItems(salidaId: ID) {
    return await query<{ medicationId: ID; qty: number; dosisRecomendada?: string; frecuencia?: string; fechaCaducidad?: string }>(`
      SELECT 
        medicamento_id as medicationId,
        cantidad as qty,
        dosis_recomendada as dosisRecomendada,
        frecuencia as frecuencia,
        fecha_caducidad as fechaCaducidad
      FROM salida_items
      WHERE salida_id = @salidaId
    `, { salidaId });
  }

  private async getCaducidadItems(caducidadId: ID) {
    // Obtener items con la fecha de caducidad del medicamento
    return await query<{ medicationId: ID; qty: number; fechaCaducidad?: string }>(`
      SELECT 
        ci.medicamento_id as medicationId,
        ci.cantidad as qty,
        m.fecha_vencimiento as fechaCaducidad
      FROM caducidad_items ci
      INNER JOIN medications m ON ci.medicamento_id = m.id
      WHERE ci.caducidad_id = @caducidadId
    `, { caducidadId });
  }

  async getEntryRequestById(id: ID): Promise<EntryRequest | null> {
    // Buscar en todas las tablas
    const entrada = await queryOne<any>(`SELECT id, 'entrada' as type FROM entradas WHERE id = @id`, { id });
    if (entrada) {
      const entry = await queryOne<any>(`
        SELECT id, folio, fecha_creacion as createdAt, estado as status, comentario as comment
        FROM entradas WHERE id = @id
      `, { id });
      if (!entry) return null;
      const items = await this.getEntradaItems(id);
      return {
        id: entry.id,
        folio: entry.folio,
        type: "entrada",
        patientId: "",
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        comment: entry.comment
      };
    }

    const salida = await queryOne<any>(`SELECT id, 'salida' as type FROM salidas WHERE id = @id`, { id });
    if (salida) {
      const entry = await queryOne<any>(`
        SELECT id, folio, paciente_id as patientId, fecha_creacion as createdAt, 
               estado as status, fecha_vencimiento as dueDate, comentario as comment
        FROM salidas WHERE id = @id
      `, { id });
      if (!entry) return null;
      const items = await this.getSalidaItems(id);
      return {
        id: entry.id,
        folio: entry.folio,
        type: "salida",
        patientId: entry.patientId,
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        dueDate: entry.dueDate,
        comment: entry.comment
      };
    }

    const caducidad = await queryOne<any>(`SELECT id, 'caducidad' as type FROM caducidades WHERE id = @id`, { id });
    if (caducidad) {
      const entry = await queryOne<any>(`
        SELECT id, folio, fecha_registro as createdAt, estado as status, comentario as comment
        FROM caducidades WHERE id = @id
      `, { id });
      if (!entry) return null;
      const items = await this.getCaducidadItems(id);
      return {
        id: entry.id,
        folio: entry.folio,
        type: "caducidad",
        patientId: "",
        createdAt: entry.createdAt,
        items,
        status: entry.status,
        comment: entry.comment
      };
    }

    return null;
  }

  async getEntryRequestByFolio(folio: string): Promise<EntryRequest | null> {
    // Buscar en todas las tablas
    const entrada = await queryOne<any>(`SELECT id FROM entradas WHERE folio = @folio`, { folio });
    if (entrada) return this.getEntryRequestById(entrada.id);

    const salida = await queryOne<any>(`SELECT id FROM salidas WHERE folio = @folio`, { folio });
    if (salida) return this.getEntryRequestById(salida.id);

    const caducidad = await queryOne<any>(`SELECT id FROM caducidades WHERE folio = @folio`, { folio });
    if (caducidad) return this.getEntryRequestById(caducidad.id);

    return null;
  }

  async createEntryRequest(data: Omit<EntryRequest, 'id' | 'folio' | 'createdAt'> & { userId?: ID; comment?: string }): Promise<EntryRequest> {
    const id = uuidv4();
    const now = new Date().toISOString();
    const folio = await this.getNextFolio(data.type);

    // Validar inventario para salidas y caducidad
    if (data.type === 'salida' || data.type === 'caducidad') {
      for (const item of data.items) {
        const medication = await queryOne<{ qty: number; name: string }>(
          'SELECT cantidad as qty, nombre as name FROM medications WHERE id = @id',
          { id: item.medicationId }
        );
        if (!medication) {
          throw new Error(`Medicamento ${item.medicationId} no encontrado`);
        }
        if (item.qty > medication.qty) {
          throw new Error(`Cantidad insuficiente de ${medication.name}. Disponible: ${medication.qty}`);
        }
      }
    }

    if (data.type === 'entrada') {
      // Crear entrada
      await execute(`
        INSERT INTO entradas (id, folio, fecha_creacion, estado, comentario, creado_por)
        VALUES (@id, @folio, @createdAt, @status, @comment, @userId)
      `, {
        id,
        folio,
        createdAt: now,
        status: data.status,
        comment: data.comment || null,
        userId: data.userId || null
      });

      // Crear items de entrada
      for (const item of data.items) {
        // Para entradas: si hay medicationName pero el medicationId es temporal o no existe,
        // buscar o crear el medicamento primero
        let actualMedicationId = item.medicationId;
        const medicationName = (item as any).medicationName;
        const medicationUnit = (item as any).unit || '';
        const fechaCaducidad = (item as any).fechaCaducidad;
        const barcode = (item as any).barcode;
        
        if (medicationName) {
          // Buscar medicamento existente por código de barras Y fecha de caducidad
          // Si tienen el mismo código pero diferente fecha, son productos diferentes
          let existingMed: { id: string; fecha_vencimiento: string } | null = null;
          
          if (barcode && fechaCaducidad) {
            // Buscar por código de barras Y fecha de caducidad
            existingMed = await queryOne<{ id: string; fecha_vencimiento: string }>(`
              SELECT id, fecha_vencimiento 
              FROM medications 
              WHERE codigo_barras = @barcode 
                AND CONVERT(DATE, fecha_vencimiento) = CONVERT(DATE, @fechaCaducidad)
            `, { barcode, fechaCaducidad });
          } else if (barcode) {
            // Si solo hay código de barras (sin fecha), buscar solo por código
            existingMed = await queryOne<{ id: string; fecha_vencimiento: string }>(`
              SELECT id, fecha_vencimiento 
              FROM medications 
              WHERE codigo_barras = @barcode
            `, { barcode });
          } else {
            // Si no hay código de barras, buscar solo por nombre
            existingMed = await queryOne<{ id: string; fecha_vencimiento: string }>(`
              SELECT id, fecha_vencimiento 
              FROM medications 
              WHERE LOWER(nombre) = LOWER(@name)
            `, { name: medicationName });
          }
          
          if (existingMed) {
            // Verificar si la fecha de caducidad es diferente
            const existingDate = existingMed.fecha_vencimiento ? new Date(existingMed.fecha_vencimiento).toISOString().split('T')[0] : null;
            const newDate = fechaCaducidad ? new Date(fechaCaducidad).toISOString().split('T')[0] : null;
            
            if (existingDate && newDate && existingDate !== newDate) {
              // Fecha de caducidad diferente: crear un nuevo medicamento
              console.log(`[SeparateEntryService] Mismo código de barras pero fecha diferente. Creando nuevo medicamento.`);
              console.log(`  Existente: ${existingDate}, Nuevo: ${newDate}`);
              const newMedication = await medicationService.createMedication({
                name: medicationName,
                qty: 0,
                unit: medicationUnit,
                expiresAt: fechaCaducidad || new Date().toISOString(),
                barcode: barcode || undefined,
                createdBy: data.userId
              });
              actualMedicationId = newMedication.id;
              console.log(`[SeparateEntryService] ✓ Nuevo medicamento creado con ID: ${actualMedicationId} (fecha diferente)`);
            } else {
              // Mismo código y misma fecha (o sin fecha): usar el medicamento existente e incrementar stock
              actualMedicationId = existingMed.id;
              console.log(`[SeparateEntryService] Usando medicamento existente: ${actualMedicationId}`);
              
              // Si se proporcionó código de barras y el medicamento no lo tiene, actualizarlo
              if (barcode) {
                const medHasBarcode = await queryOne<{ codigo_barras: string }>(`
                  SELECT codigo_barras FROM medications WHERE id = @id
                `, { id: actualMedicationId });
                
                if (!medHasBarcode?.codigo_barras) {
                  await execute(`
                    UPDATE medications 
                    SET codigo_barras = @barcode, fecha_actualizacion = @updatedAt
                    WHERE id = @id
                  `, { id: actualMedicationId, barcode, updatedAt: now });
                  console.log(`[SeparateEntryService] ✓ Código de barras agregado al medicamento existente`);
                }
              }
            }
          } else {
            // No existe: crear un nuevo medicamento
            console.log(`[SeparateEntryService] Creando nuevo medicamento: ${medicationName}, barcode: ${barcode || 'sin código'}, fecha: ${fechaCaducidad || 'sin fecha'}`);
            const newMedication = await medicationService.createMedication({
              name: medicationName,
              qty: 0, // Se actualizará después con la cantidad de la entrada
              unit: medicationUnit,
              expiresAt: fechaCaducidad || new Date().toISOString(),
              barcode: barcode || undefined,
              createdBy: data.userId
            });
            actualMedicationId = newMedication.id;
            console.log(`[SeparateEntryService] ✓ Medicamento creado con ID: ${actualMedicationId}`);
          }
        }
        
        // Obtener nombre y unidad para guardar (siempre guardar para mantener historial)
        let medicationNameToSave = medicationName;
        let unitToSave = medicationUnit;
        
        // Obtener dosis sugerida del item
        const dosisSugerida = (item as any).dosisRecomendada || null;
        
        // Si no se proporcionó medicationName pero el medicamento existe, obtenerlo de la tabla medications
        if (!medicationNameToSave && actualMedicationId) {
          const medInfo = await queryOne<{ nombre: string; unidad: string }>(`
            SELECT nombre, unidad FROM medications WHERE id = @id
          `, { id: actualMedicationId });
          if (medInfo) {
            medicationNameToSave = medInfo.nombre;
            unitToSave = medInfo.unidad || unitToSave;
          }
        }
        
        // Log para depuración
        console.log('[SeparateEntryService] Guardando item de entrada:', {
          medicationId: actualMedicationId,
          medicationName: medicationNameToSave,
          unit: unitToSave,
          qty: item.qty,
          fechaCaducidad: fechaCaducidad,
          dosisSugerida: dosisSugerida
        });
        
        try {
          // Intentar insertar con nombre_medicamento, unidad, fecha_caducidad y dosis_sugerida
          await execute(`
            INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad, nombre_medicamento, unidad, fecha_caducidad, dosis_sugerida)
            VALUES (@itemId, @entradaId, @medicationId, @qty, @medicationName, @unit, @fechaCaducidad, @dosisSugerida)
          `, {
            itemId: uuidv4(),
            entradaId: id,
            medicationId: actualMedicationId,
            qty: item.qty,
            medicationName: medicationNameToSave || null,
            unit: unitToSave || null,
            fechaCaducidad: fechaCaducidad || null,
            dosisSugerida: dosisSugerida
          });
          console.log('[SeparateEntryService] ✓ Item insertado correctamente');
        } catch (error: any) {
          // Si falla por columnas faltantes, intentar sin dosis_sugerida
          if (error.message && (error.message.includes('dosis_sugerida') || error.message.includes('Invalid column name'))) {
            try {
              await execute(`
                INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad, nombre_medicamento, unidad, fecha_caducidad)
                VALUES (@itemId, @entradaId, @medicationId, @qty, @medicationName, @unit, @fechaCaducidad)
              `, {
                itemId: uuidv4(),
                entradaId: id,
                medicationId: actualMedicationId,
                qty: item.qty,
                medicationName: medicationNameToSave || null,
                unit: unitToSave || null,
                fechaCaducidad: fechaCaducidad || null
              });
              console.warn('⚠️ Columna dosis_sugerida no existe en entrada_items. Insertando sin ella.');
            } catch (error2: any) {
              // Si también falla por otras columnas, insertar solo lo básico
              if (error2.message && (error2.message.includes('nombre_medicamento') || error2.message.includes('unidad') || error2.message.includes('fecha_caducidad') || error2.message.includes('Invalid column name'))) {
                console.warn('⚠️ Columnas nombre_medicamento, unidad o fecha_caducidad no existen en entrada_items. Insertando sin ellas.');
                await execute(`
                  INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad)
                  VALUES (@itemId, @entradaId, @medicationId, @qty)
                `, {
                  itemId: uuidv4(),
                  entradaId: id,
                  medicationId: actualMedicationId,
                  qty: item.qty
                });
              } else {
                throw error2;
              }
            }
          } else {
            throw error;
          }
        }

        // Actualizar cantidad y dosis del medicamento
        if (dosisSugerida) {
          // Si hay dosis sugerida, actualizar cantidad y dosis
          await execute(`
            UPDATE medications
            SET cantidad = cantidad + @qty, 
                dosis = @dosis, 
                fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, { 
            medicationId: actualMedicationId, 
            qty: item.qty, 
            dosis: dosisSugerida,
            updatedAt: now 
          });
          console.log('[SeparateEntryService] ✓ Cantidad y dosis actualizadas en medicamento');
        } else {
          // Si no hay dosis, solo actualizar cantidad
          await execute(`
            UPDATE medications
            SET cantidad = cantidad + @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, { medicationId: actualMedicationId, qty: item.qty, updatedAt: now });
        }
      }
    } else if (data.type === 'salida') {
      // Validar que tenga paciente
      if (!data.patientId) {
        throw new Error('patientId es requerido para salidas');
      }

      // Crear salida
      await execute(`
        INSERT INTO salidas (id, folio, paciente_id, fecha_creacion, estado, fecha_vencimiento, comentario, creado_por)
        VALUES (@id, @folio, @patientId, @createdAt, @status, @dueDate, @comment, @userId)
      `, {
        id,
        folio,
        patientId: data.patientId,
        createdAt: now,
        status: data.status,
        dueDate: data.dueDate || null,
        comment: data.comment || null,
        userId: data.userId || null
      });

      // Crear items de salida
      for (const item of data.items) {
        await execute(`
          INSERT INTO salida_items (id, salida_id, medicamento_id, cantidad, dosis_recomendada, frecuencia, fecha_caducidad)
          VALUES (@itemId, @salidaId, @medicationId, @qty, @dosis, @frecuencia, @fechaCaducidad)
        `, {
          itemId: uuidv4(),
          salidaId: id,
          medicationId: item.medicationId,
          qty: item.qty,
          dosis: item.dosisRecomendada || null,
          frecuencia: item.frecuencia || null,
          fechaCaducidad: item.fechaCaducidad || null
        });

        // Disminuir cantidad del medicamento
        await execute(`
          UPDATE medications
          SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
          WHERE id = @medicationId
        `, { medicationId: item.medicationId, qty: item.qty, updatedAt: now });

        // Registrar en patient_medications si tiene dosis y frecuencia
        if (item.dosisRecomendada && item.frecuencia) {
          try {
            await patientMedicationService.assignMedication({
              patientId: data.patientId!,
              medicationId: item.medicationId,
              dosage: item.dosisRecomendada,
              frequency: item.frecuencia,
              prescribedAt: now,
              prescribedBy: data.userId || undefined,
              cantidad: item.qty
            });
          } catch (error) {
            console.error('Error al registrar medicamento en patient_medications:', error);
          }
        }
      }
    } else if (data.type === 'caducidad') {
      // Caducidad NO requiere paciente
      
      // Crear caducidad (fecha_registro se establece automáticamente con GETDATE())
      await execute(`
        INSERT INTO caducidades (id, folio, fecha_registro, estado, comentario, creado_por)
        VALUES (@id, @folio, @fechaRegistro, @status, @comment, @userId)
      `, {
        id,
        folio,
        fechaRegistro: now,
        status: data.status,
        comment: data.comment || null,
        userId: data.userId || null
      });

      // Crear items de caducidad (la fecha de caducidad se obtiene del medicamento)
      for (const item of data.items) {
        // Obtener la fecha de caducidad del medicamento
        const medication = await queryOne<{ expiresAt: string }>(
          'SELECT fecha_vencimiento as expiresAt FROM medications WHERE id = @id',
          { id: item.medicationId }
        );
        
        await execute(`
          INSERT INTO caducidad_items (id, caducidad_id, medicamento_id, cantidad, fecha_caducidad)
          VALUES (@itemId, @caducidadId, @medicationId, @qty, @fechaCaducidad)
        `, {
          itemId: uuidv4(),
          caducidadId: id,
          medicationId: item.medicationId,
          qty: item.qty,
          fechaCaducidad: medication?.expiresAt || item.fechaCaducidad || null
        });

        // Disminuir cantidad del medicamento
        await execute(`
          UPDATE medications
          SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
          WHERE id = @medicationId
        `, { medicationId: item.medicationId, qty: item.qty, updatedAt: now });
      }
    }

    const result = await this.getEntryRequestById(id);
    if (!result) {
      throw new Error('Error al crear la solicitud: no se pudo recuperar después de la creación');
    }
    return result;
  }

  async updateEntryRequest(id: ID, data: Partial<EntryRequest>): Promise<EntryRequest | null> {
    const existing = await this.getEntryRequestById(id);
    if (!existing) return null;

    if (existing.type === 'entrada') {
      await execute(`
        UPDATE entradas 
        SET estado = COALESCE(@status, estado),
            comentario = COALESCE(@comment, comentario)
        WHERE id = @id
      `, {
        status: data.status || null,
        comment: (data as any).comment || null,
        id
      });
    } else if (existing.type === 'salida') {
      await execute(`
        UPDATE salidas 
        SET paciente_id = COALESCE(@patientId, paciente_id),
            estado = COALESCE(@status, estado),
            fecha_vencimiento = COALESCE(@dueDate, fecha_vencimiento),
            comentario = COALESCE(@comment, comentario)
        WHERE id = @id
      `, {
        patientId: data.patientId || null,
        status: data.status || null,
        dueDate: data.dueDate || null,
        comment: (data as any).comment || null,
        id
      });
    } else if (existing.type === 'caducidad') {
      await execute(`
        UPDATE caducidades 
        SET estado = COALESCE(@status, estado),
            comentario = COALESCE(@comment, comentario)
        WHERE id = @id
      `, {
        status: data.status || null,
        comment: (data as any).comment || null,
        id
      });
    }

    if (data.items) {
      const now = new Date().toISOString();
      
      // Eliminar items existentes y crear nuevos
      if (existing.type === 'entrada') {
        // Obtener items antiguos antes de eliminarlos para revertir el inventario
        const oldItems = await this.getEntradaItems(id);
        
        // Revertir el inventario: restar las cantidades antiguas
        for (const oldItem of oldItems) {
          await execute(`
            UPDATE medications
            SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, {
            medicationId: oldItem.medicationId,
            qty: oldItem.qty,
            updatedAt: now
          });
        }
        
        // Eliminar items antiguos
        await execute('DELETE FROM entrada_items WHERE entrada_id = @id', { id });
        
        // Insertar nuevos items y actualizar inventario
        for (const item of data.items) {
          const dosisSugerida = (item as any).dosisRecomendada || null;
          const medicationName = (item as any).medicationName || null;
          const unit = (item as any).unit || null;
          const fechaCaducidad = (item as any).fechaCaducidad || null;
          
          try {
            // Intentar insertar con todas las columnas
            await execute(`
              INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad, nombre_medicamento, unidad, fecha_caducidad, dosis_sugerida)
              VALUES (@itemId, @entradaId, @medicationId, @qty, @medicationName, @unit, @fechaCaducidad, @dosisSugerida)
            `, {
              itemId: uuidv4(),
              entradaId: id,
              medicationId: item.medicationId,
              qty: item.qty,
              medicationName: medicationName,
              unit: unit,
              fechaCaducidad: fechaCaducidad,
              dosisSugerida: dosisSugerida
            });
          } catch (error: any) {
            // Si falla por columnas faltantes, intentar sin dosis_sugerida
            if (error.message && error.message.includes('dosis_sugerida')) {
              try {
                await execute(`
                  INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad, nombre_medicamento, unidad, fecha_caducidad)
                  VALUES (@itemId, @entradaId, @medicationId, @qty, @medicationName, @unit, @fechaCaducidad)
                `, {
                  itemId: uuidv4(),
                  entradaId: id,
                  medicationId: item.medicationId,
                  qty: item.qty,
                  medicationName: medicationName,
                  unit: unit,
                  fechaCaducidad: fechaCaducidad
                });
              } catch (error2: any) {
                // Si también falla, insertar solo lo básico
                if (error2.message && (error2.message.includes('nombre_medicamento') || error2.message.includes('unidad') || error2.message.includes('fecha_caducidad'))) {
                  await execute(`
                    INSERT INTO entrada_items (id, entrada_id, medicamento_id, cantidad)
                    VALUES (@itemId, @entradaId, @medicationId, @qty)
                  `, {
                    itemId: uuidv4(),
                    entradaId: id,
                    medicationId: item.medicationId,
                    qty: item.qty
                  });
                } else {
                  throw error2;
                }
              }
            } else {
              throw error;
            }
          }
          
          // Actualizar inventario: sumar las nuevas cantidades
          if (dosisSugerida) {
            await execute(`
              UPDATE medications
              SET cantidad = cantidad + @qty, 
                  dosis = @dosis, 
                  fecha_actualizacion = @updatedAt
              WHERE id = @medicationId
            `, {
              medicationId: item.medicationId,
              qty: item.qty,
              dosis: dosisSugerida,
              updatedAt: now
            });
          } else {
            await execute(`
              UPDATE medications
              SET cantidad = cantidad + @qty, fecha_actualizacion = @updatedAt
              WHERE id = @medicationId
            `, {
              medicationId: item.medicationId,
              qty: item.qty,
              updatedAt: now
            });
          }
        }
      } else if (existing.type === 'salida') {
        // Obtener items antiguos antes de eliminarlos para revertir el inventario
        const oldItems = await this.getSalidaItems(id);
        
        // Revertir el inventario: sumar las cantidades antiguas (retornar al inventario)
        for (const oldItem of oldItems) {
          await execute(`
            UPDATE medications
            SET cantidad = cantidad + @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, {
            medicationId: oldItem.medicationId,
            qty: oldItem.qty,
            updatedAt: now
          });
        }
        
        // Eliminar items antiguos
        await execute('DELETE FROM salida_items WHERE salida_id = @id', { id });
        
        // Insertar nuevos items y actualizar inventario
        for (const item of data.items) {
          await execute(`
            INSERT INTO salida_items (id, salida_id, medicamento_id, cantidad, dosis_recomendada, frecuencia, fecha_caducidad)
            VALUES (@itemId, @salidaId, @medicationId, @qty, @dosis, @frecuencia, @fechaCaducidad)
          `, {
            itemId: uuidv4(),
            salidaId: id,
            medicationId: item.medicationId,
            qty: item.qty,
            dosis: item.dosisRecomendada || null,
            frecuencia: item.frecuencia || null,
            fechaCaducidad: item.fechaCaducidad || null
          });
          
          // Actualizar inventario: restar las nuevas cantidades
          await execute(`
            UPDATE medications
            SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, {
            medicationId: item.medicationId,
            qty: item.qty,
            updatedAt: now
          });
        }
      } else if (existing.type === 'caducidad') {
        // Obtener items antiguos antes de eliminarlos para revertir el inventario
        const oldItems = await this.getCaducidadItems(id);
        
        // Revertir el inventario: sumar las cantidades antiguas (retornar al inventario)
        for (const oldItem of oldItems) {
          await execute(`
            UPDATE medications
            SET cantidad = cantidad + @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, {
            medicationId: oldItem.medicationId,
            qty: oldItem.qty,
            updatedAt: now
          });
        }
        
        // Eliminar items antiguos
        await execute('DELETE FROM caducidad_items WHERE caducidad_id = @id', { id });
        
        // Insertar nuevos items y actualizar inventario
        for (const item of data.items) {
          // Obtener la fecha de caducidad del medicamento
          const medication = await queryOne<{ expiresAt: string }>(
            'SELECT fecha_vencimiento as expiresAt FROM medications WHERE id = @id',
            { id: item.medicationId }
          );
          
          await execute(`
            INSERT INTO caducidad_items (id, caducidad_id, medicamento_id, cantidad, fecha_caducidad)
            VALUES (@itemId, @caducidadId, @medicationId, @qty, @fechaCaducidad)
          `, {
            itemId: uuidv4(),
            caducidadId: id,
            medicationId: item.medicationId,
            qty: item.qty,
            fechaCaducidad: medication?.expiresAt || item.fechaCaducidad || null
          });
          
          // Actualizar inventario: restar las nuevas cantidades
          await execute(`
            UPDATE medications
            SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
            WHERE id = @medicationId
          `, {
            medicationId: item.medicationId,
            qty: item.qty,
            updatedAt: now
          });
        }
      }
    }

    return await this.getEntryRequestById(id);
  }

  // Verificar si una entrada tiene salidas asociadas
  async entradaHasSalidas(entradaId: ID): Promise<boolean> {
    try {
      // Obtener los medicamentos de la entrada
      const entradaItems = await query<{ medicationId: ID }>(`
        SELECT medicamento_id as medicationId
        FROM entrada_items
        WHERE entrada_id = @entradaId
      `, { entradaId });

      if (entradaItems.length === 0) return false;

      // Obtener la fecha de la entrada
      const entrada = await queryOne<{ createdAt: string }>(`
        SELECT fecha_creacion as createdAt
        FROM entradas
        WHERE id = @entradaId
      `, { entradaId });

      if (!entrada) return false;

      // Verificar si alguno de estos medicamentos fue usado en salidas
      // después de la fecha de la entrada
      const medicationIds = entradaItems.map(item => item.medicationId);
      
      // Escapar IDs para prevenir SQL injection
      const medicationIdsStr = medicationIds.map(id => `'${String(id).replace(/'/g, "''")}'`).join(',');
      
      // Usar EXISTS para mejor rendimiento
      const result = await queryOne<{ count: number }>(`
        SELECT COUNT(*) as count
        FROM salidas s
        INNER JOIN salida_items si ON s.id = si.salida_id
        WHERE si.medicamento_id IN (${medicationIdsStr})
          AND s.fecha_creacion >= @entradaDate
      `, { entradaDate: entrada.createdAt });

      return (result?.count || 0) > 0;
    } catch (error) {
      console.error('Error verificando salidas asociadas:', error);
      // En caso de error, asumir que sí tiene salidas para ser conservador
      return true;
    }
  }

  async deleteEntryRequest(id: ID): Promise<boolean> {
    const existing = await this.getEntryRequestById(id);
    if (!existing) return false;

    // Validaciones según el tipo
    if (existing.type === 'caducidad') {
      throw new Error('No se puede eliminar un registro de caducidad. Son registros históricos permanentes.');
    }

    if (existing.type === 'entrada') {
      // Verificar si tiene salidas asociadas
      const hasSalidas = await this.entradaHasSalidas(id);
      if (hasSalidas) {
        throw new Error('No se puede eliminar esta entrada porque ya se originaron salidas a partir de ella.');
      }
      
      // Si no tiene salidas, eliminar y revertir inventario
      const items = await this.getEntradaItems(id);
      const now = new Date().toISOString();
      
      for (const item of items) {
        // Obtener la cantidad actual del medicamento antes de restar
        const medication = await queryOne<{ cantidad: number; nombre: string }>(`
          SELECT cantidad, nombre FROM medications WHERE id = @medicationId
        `, { medicationId: item.medicationId });
        
        if (medication) {
          const nuevaCantidad = medication.cantidad - item.qty;
          
          // Si la cantidad resultante es 0 o negativa, verificar si el medicamento tiene otras referencias
          if (nuevaCantidad <= 0) {
            // Verificar si el medicamento tiene otras entradas o salidas
            const otrasEntradas = await queryOne<{ count: number }>(`
              SELECT COUNT(*) as count
              FROM entrada_items ei
              INNER JOIN entradas e ON ei.entrada_id = e.id
              WHERE ei.medicamento_id = @medicationId
                AND e.id != @entradaId
            `, { medicationId: item.medicationId, entradaId: id });
            
            const otrasSalidas = await queryOne<{ count: number }>(`
              SELECT COUNT(*) as count
              FROM salida_items si
              INNER JOIN salidas s ON si.salida_id = s.id
              WHERE si.medicamento_id = @medicationId
            `, { medicationId: item.medicationId });
            
            const otrasCaducidades = await queryOne<{ count: number }>(`
              SELECT COUNT(*) as count
              FROM caducidad_items ci
              INNER JOIN caducidades c ON ci.caducidad_id = c.id
              WHERE ci.medicamento_id = @medicationId
            `, { medicationId: item.medicationId });
            
            // Si no tiene otras referencias, eliminar el medicamento completamente
            if ((otrasEntradas?.count || 0) === 0 && (otrasSalidas?.count || 0) === 0 && (otrasCaducidades?.count || 0) === 0) {
              console.log(`[SeparateEntryService] Eliminando medicamento ${medication.nombre} (ID: ${item.medicationId}) porque no tiene otras referencias y su cantidad sería 0`);
              await execute('DELETE FROM medications WHERE id = @medicationId', { medicationId: item.medicationId });
            } else {
              // Si tiene otras referencias, solo actualizar la cantidad a 0
              await execute(`
                UPDATE medications
                SET cantidad = 0, fecha_actualizacion = @updatedAt
                WHERE id = @medicationId
              `, {
                medicationId: item.medicationId,
                updatedAt: now
              });
            }
          } else {
            // Si la cantidad es positiva, solo restar
            await execute(`
              UPDATE medications
              SET cantidad = cantidad - @qty, fecha_actualizacion = @updatedAt
              WHERE id = @medicationId
            `, {
              medicationId: item.medicationId,
              qty: item.qty,
              updatedAt: now
            });
          }
        }
      }
      
      // Eliminar items primero
      await execute('DELETE FROM entrada_items WHERE entrada_id = @id', { id });
      // Eliminar entrada
      const deleted = await execute('DELETE FROM entradas WHERE id = @id', { id });
      return deleted > 0;
    }

    if (existing.type === 'salida') {
      // Retornar inventario al eliminar salida
      const items = await this.getSalidaItems(id);
      for (const item of items) {
        // Aumentar cantidad del medicamento (retornar al inventario)
        await execute(`
          UPDATE medications
          SET cantidad = cantidad + @qty, fecha_actualizacion = @updatedAt
          WHERE id = @medicationId
        `, { 
          medicationId: item.medicationId, 
          qty: item.qty, 
          updatedAt: new Date().toISOString() 
        });
      }
      
      // Eliminar items primero
      await execute('DELETE FROM salida_items WHERE salida_id = @id', { id });
      // Eliminar salida
      const deleted = await execute('DELETE FROM salidas WHERE id = @id', { id });
      return deleted > 0;
    }

    return false;
  }
}

export const separateEntryService = new SeparateEntryService();

